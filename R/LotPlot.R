...
#' @param hi_col An R-compatible colour to use for the lowest expressed features. Defaults to "white".
#' @param mid_col An R-compatible colour to use for the middest expressed features (for scaled data, this will be the 0 colour). Defaults to "lightsteelblue1".
#' @param lo_col An R-compatible colour to use for the highest expressed features. Defaults to "midnightblue".
...
#'   "Memory" = 15:20)
#' # Usage:
#' # p <- LotPlot(seurat_object,
#' #            gene_groups = gene_groups)
#' # p
#'
#' # EXAMPLE 2: Custom groupings format
#' # You can also use the length() and unlist() to convert a list of concatenated characters:
...
#' # Usage:
#' p <- LotPlot(seurat_object, features = unlist(c(list_1, list_2, list_3)), gene_groups)
#' p
#' }
#' @export

utils::globalVariables(c("features.plot", "pct.exp", "avg.exp.scaled"))

LotPlot <- function(seurat_object, gene_groups = NULL, group_size = 5, ..., lo_col = NULL, mid_col = NULL, hi_col = NULL){

  p <- Seurat::DotPlot(seurat_object, ...) + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
  #p
